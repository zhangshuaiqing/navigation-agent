...
| 批次 | 功能 | 实际代码量 | 实际耗时 | 关键发现 |
|------|------|-----------|---------|---------|
| A | 功能 1: 随机起点/终点 | ~55 行 | 15 min | 无 |
| A | 功能 2: 局部视野 (full/local/fog_of_war) | ~130 行 | 30 min | Heuristic Agent 在无 BFS hint 下会徘徊，需增加方向引导+循环惩罚 |
| B | 功能 3: 动态障碍物 (bounce/wrap/random) | ~180 行 | 45 min | bounce 模式越界时未刷新坐标导致 IndexError |
| C | 功能 4: 多目标任务 (sequential/any_order/collect) | ~170 行 | 60 min | 需确保 sequential 模式下目标间路径可达 |

### 修复的 Bug

| Bug | 文件 | 原因 | 修复 |
|-----|------|------|------|
| Heuristic Agent 在 local/fog 下徘徊 | `navigator.py` | 无方向引导, 无循环检测 | +goal_direction 奖励, +recent 3 步惩罚惩罚 |
| `index 8 is out of bounds for axis 0` | `gridworld.py` | bounce 仅反转方向未重算坐标 | 反转方向后立即重算 nr,nc |
| DynamicObstacle 越界残留 | `gridworld.py` | bounce 后未 continue, 继续用旧越界值访问 grid | bounce 后仍越界则 continue |

### 当前项目规模

```
git log --oneline | wc -l:  14 commits
git ls-files | wc -l:       24 files tracked
```

### 环境参数总表

| 参数 | CLI | Web UI | 默认值 | 可选值 |
|------|-----|--------|--------|--------|
| 网格大小 | `--size` | Grid Size | 8 | 4~20 |
| 障碍物比例 | `--obstacles` | Obstacle Ratio | 0.2 | 0.0~0.5 |
| 随机种子 | `--seed` | Random Seed | random | 任意整数 |
| 随机起点/终点 | `--random-start-goal` | Random Start/Goal | off | on/off |
| 观测模式 | `--obs-mode` | Observation Mode | full | full/local/fog_of_war |
| 视野范围 | `--view-range` | View Range | 1 | 1~5 |
| 动态障碍物数 | `--dynamic` | Dynamic Obstacles | 0 | 0~10 |
| 目标数量 | `--goals` | Number of Goals | 1 | 1~10 |
| 任务类型 | `--task-type` | Task Type | sequential | sequential/any_order/collect |
